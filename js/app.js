const apiUrl = "https://smtsh44.freshdesk.com/api/v2/";
var currentPage = "";
const token = "Wn5LbvPUHUtvjsxI5l9U:X";
...
async function routeToLink(route, isPushState = true) {
    ...
    for (let r of router) {
        ...
        if (
            r_index != 0 &&
            route_index != 0 &&
            r.path.slice(0, r_index) == route.slice(0, route_index)
        ) {
...
async function asyncGetFetchRequest(resourceName, queryParams) {
    ...
    let options = {
        ...
        headers: {
            Authorization: "Baisc " + btoa(token),
            "Content-Type": "application/json",
        },
    };

    await fetch(
        apiUrl + resourceName + (queryParams ? "?" + queryParams : ""),
        options
    )
        .then((resp) => resp.json())
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let sidenav_instance = M.Sidenav.getInstance(
